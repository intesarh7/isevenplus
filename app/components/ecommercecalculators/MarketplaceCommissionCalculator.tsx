"use client";

import { useState } from "react";
import { Store, RotateCcw, PlayCircle, Calculator } from "lucide-react";

export default function MarketplaceCommissionCalculator() {
    const [sellingPrice, setSellingPrice] = useState("");
    const [commissionRate, setCommissionRate] = useState("");
    const [otherFees, setOtherFees] = useState("");

    const [commissionAmount, setCommissionAmount] = useState<number | null>(null);
    const [netPayout, setNetPayout] = useState<number | null>(null);

    const calculateCommission = () => {
        const price = parseFloat(sellingPrice);
        const rate = parseFloat(commissionRate);
        const extraFees = parseFloat(otherFees) || 0;

        if (isNaN(price) || isNaN(rate)) {
            setCommissionAmount(null);
            return;
        }

        const commission = (price * rate) / 100;
        const payout = price - commission - extraFees;

        setCommissionAmount(parseFloat(commission.toFixed(2)));
        setNetPayout(parseFloat(payout.toFixed(2)));
    };

    const tryExample = () => {
        setSellingPrice("2500");
        setCommissionRate("18");
        setOtherFees("150");

        const commission = (2500 * 18) / 100;
        const payout = 2500 - commission - 150;

        setCommissionAmount(commission);
        setNetPayout(payout);
    };

    const resetFields = () => {
        setSellingPrice("");
        setCommissionRate("");
        setOtherFees("");
        setCommissionAmount(null);
        setNetPayout(null);
    };

    return (
        <div className="max-w-4xl mx-auto bg-white shadow-md rounded-xl p-6 md:p-8">

            <h2 className="text-2xl font-bold flex items-center gap-2 mb-6">
                <Store className="text-green-600" />
                Marketplace Commission Calculator
            </h2>

            <div className="grid md:grid-cols-2 gap-6">

                <input
                    type="number"
                    placeholder="Selling Price (₹)"
                    value={sellingPrice}
                    onChange={(e) => setSellingPrice(e.target.value)}
                    className="border rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500"
                />

                <input
                    type="number"
                    placeholder="Commission Rate (%)"
                    value={commissionRate}
                    onChange={(e) => setCommissionRate(e.target.value)}
                    className="border rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500"
                />

                <input
                    type="number"
                    placeholder="Other Marketplace Fees (₹)"
                    value={otherFees}
                    onChange={(e) => setOtherFees(e.target.value)}
                    className="border rounded-lg px-4 py-3 focus:ring-2 focus:ring-indigo-500 md:col-span-2"
                />

            </div>

            <div className="flex flex-col sm:flex-row gap-4 mt-6">

                <button
                    onClick={calculateCommission}
                    className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-3 rounded-lg flex items-center justify-center gap-2"
                >
                    <Calculator size={18} /> Calculate
                </button>

                <button
                    onClick={tryExample}
                    className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg flex items-center justify-center gap-2"
                >
                    <PlayCircle size={18} /> Try Example
                </button>

                <button
                    onClick={resetFields}
                    className="flex-1 bg-gray-500 hover:bg-gray-600 text-white py-3 rounded-lg flex items-center justify-center gap-2"
                >
                    <RotateCcw size={18} /> Reset
                </button>

            </div>

            {commissionAmount !== null && (
                <div className="mt-8 bg-indigo-50 p-6 rounded-lg text-center space-y-2">
                    <p className="text-lg font-semibold">
                        Commission Amount: ₹ {commissionAmount}
                    </p>
                    <p className="text-2xl font-bold text-indigo-600">
                        Net Payout: ₹ {netPayout}
                    </p>
                </div>
            )}
            <div className="mt-16">

                <h2 className="text-2xl font-bold mb-4">
                    How Marketplace Commission is Calculated?
                </h2>

                <p className="mb-4 text-gray-700 leading-relaxed">
                    Online marketplaces charge a percentage commission on each sale.
                    Sellers also pay additional fixed or variable fees.
                </p>

                <div className="bg-gray-100 p-4 rounded-lg mb-6 text-center font-semibold">
                    Net Payout = Selling Price − Commission − Other Fees
                </div>

                <h3 className="text-xl font-semibold mb-3">
                    Why This Calculator is Useful?
                </h3>

                <ul className="list-disc pl-6 space-y-2 text-gray-700">
                    <li>Helps estimate real seller payout</li>
                    <li>Improves pricing strategy</li>
                    <li>Reduces hidden margin loss</li>
                    <li>Supports marketplace sellers</li>
                </ul>

            </div>
        </div>
    );
}